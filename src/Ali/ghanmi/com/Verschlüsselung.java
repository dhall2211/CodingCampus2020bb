...
public class Verschlüsselung {
    public final static String buch = "Zur Zeit des Zweiten Weltkriegs waren seine großen Werke Siddhartha und Der Steppenwolf noch verboten. Heute gehört Hermann Hesse zu den bekanntesten deutschen Schriftstellern. Mehr über den Weltveränderer lest ihr hier\n" +
            "Hermann Hesse\n" +
            "\n" +
    ...
    public static String encryption (String text) {
        ...
        for (int i = 0; i < text.length(); i++) {
            ...
            //System.out.println(buch.inddexOf(text.charAt(i)));
            position = (buch.indexOf(text.charAt(i),position));
    ...
    public static String decryption (String encryptedText) {
        ...
        int lastPositioon=0;
        String [] wordes=encryptedText.split( " ");

        //System.out.println();
        for (int i = 0; i <wordes.length ; i++) {
            int position = Integer.valueOf(wordes[i]);
            decryptionText += (buch.charAt(position + lastPositioon));
          lastPositioon = position +lastPositioon;
        }
